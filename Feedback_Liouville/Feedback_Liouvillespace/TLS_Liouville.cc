...
//         Print(temp);
        ...
        G2_Refbin_Op.set(refbin(1), prime(refbin(4)), 1.0);
        
        ITensor G2_Timebin_Op = ITensor(timebin, prime(timebin));
        G2_Timebin_Op.set(timebin(4), prime(timebin(4)), 1.0);    
        // calculate g(2) in density matrix picture:
        ITensor g2_tensor = dag(temp)*noprime(G2_Timebin_Op*(noprime(G2_Refbin_Op*temp)));
        std::complex<double> g2 = g2_tensor.cplx();
        //g2 /= pow(2,ttotal);
        //g2 = g2.real();
        //g2 = pow(g2,0.5);
        return g2;
...
//         Uevo = identity_int*identity_system + U_1+U_2+U_3+U_4+U_5+U_6+U_7+U_8+U_9+U_10;
       ...
       //Uevo = identity_int*identity_system + Lsys + Lint;
...
//         Sp_RefBin.set(bin[referencetimebin](2), prime(bin[referencetimebin])(4), 1.0);
        ...
        g1_bath[0] = TLS_Liouville::excited_occupation(psi.A(referencetimebin), Site);
        g2_bath[0] = 0.0;
        pop_bath[0] = TLS_Liouville::excited_occupation(psi.A(referencetimebin), Site);
...
//         }
        ...
        for(int i=referencetimebin;i>(referencetimebin-spectrum_steps);i--)
        {
            ...
            pop_bath[k] = TLS_Liouville::excited_occupation(psi.A(i), Site);

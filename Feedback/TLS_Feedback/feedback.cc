...
namespace itensor{
    ...
    double Feedback::TLS_occupation(ITensor psitensor, IndexType type)
    {
        ...
        return occ;
        //return (dag(prime(psi, Site)) * (SpSm*psi)).real();
    }
    ...
    {
        // index_system = IndexType("system");
        // index_timebin = IndexType("timebin");
        // index_link = IndexType("link");
        
        bin = std::vector<Index>(ttotal+1);
        // linkbin = std::vector<Index>(ttotal+1);
        ...
        for (int i=1;i<(ttotal+1);i++)
        {
            ...
            //linkbin.at(i) = Index(nameint("l",i),1,link);
        }
    }
...
//         Uevo = identity_system*identity_int*identity_fb + Hsys + Hint + 0.5*mapprime(prime(Hint)*Hint,2,1);
    ...
    void Feedback::timeevolve_system_with_feedback()
    {
        ...
        for(int j=0; j<Nfb; j++)
        {
            ...
            if ((j%1)==0) 
            {
                printf("time %.3f of %.3f -- norm=%.10f -- occ=%.10f \n", j*dt, t_end, (1.0 - Feedback::norm(psi.A(j+2), Site)), Feedback::TLS_occupation(psi.A(j+2), Site));
            }
        }
        // OC is now at A(Nfb+2), which is system. Shift it to first feedbackbin
        ITensor U,S,V,temp;
        for(int i=Nfb+2; i>1; i--) 
        {
            Index oc_timebin = findtype(psi.A(i-1),Site);
            temp = psi.A(i-1)*psi.A(i);
            U = ITensor(oc_timebin, leftLinkInd(psi,i-1));
            svd(temp,U,S,V,args);
            psi.setA(i,V);
            psi.setA(i-1,U*S);
        }
    ...
    void Feedback::apply_Uevo_with_feedback(int timestep)
    {
        ...
        fprintf(datastream, "%f \t %e \t %e \n", timestep*dt, Feedback::TLS_occupation(psi.A(timestep+1), Site), (1.0 - Feedback::norm(psi.A(timestep+1), Site)));
        if ((timestep%10)==0) 
        {
            printf("time %.3f of %.3f -- norm=%.10f \n",timestep*dt,t_end*1., (1.0 - Feedback::norm(psi.A(timestep+1), Site)));
        }
        ...
        // and feedbacktimebin back to A(timestep) - it does not change its position!

...
namespace itensor{
    ...
    double TLS_Liouville::norm(ITensor psi, IndexType type)
    {
    ...
    // V becomes new systembin at next timestep in MPS
    {
        ...
        printf("time %.3f of %.3f -- norm=%.10f -- occ=%.10f \n", 0.0*dt, t_end, (1.0 - TLS_Liouville::norm(psi.A(1), Site)), TLS_Liouville::TLS_occupation(psi.A(1), Site));
        
        TLS_Liouville::set_up_operator();
        for(int j = 0; j<ttotal; j++)
        {
            fprintf(datastream, "%f \t %e \t %e \n", j*dt, TLS_Liouville::TLS_occupation(psi.A(j+1), Site), (1.0 - TLS_Liouville::norm(psi.A(j+1), Site)));
            ...
            if ((j%10)==0) 
            {
                printf("time %.3f of %.3f -- norm=%.10f \n", j*dt, t_end*1., (1.0 - TLS_Liouville::norm(psi.A(j+2), Site)));
            }
        }

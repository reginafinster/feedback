...
printf("Date: %d.%d.%.d -- Time: %d:%d:%d  \n",day,month+1,year,hour,minute,second);  
    ...
    Real fb_fak = input.getReal("fb_fak");
...
double norm_psi = norm(psi); printf("Norm(psi)=%.5f\n",norm_psi);
    ...
    {            
        U_dis.set(bin[1](2),prime(bin[1](1)),bin[2](phot),prime(bin[2](phot+1)),( 1.0)*sqrt(dt)*Gamma*sqrt(phot));
        U_dis.set(bin[1](1),prime(bin[1](2)),bin[2](phot+1),prime(bin[2](phot)),(-1.0)*sqrt(dt)*Gamma*sqrt(phot));
    }//phot
...
// ---- erzeuge neuen zeitevolutionsoperator fuer den Feedbackteil    
    ...
    for(int phot=1;phot<Nbin;phot++)
    {            
        U_disfb.set(bin[1](2),prime(bin[1](1)),bin[Nfb+1](phot  ),prime(bin[Nfb+1](phot+1)),( 1.)*sqrt(dt)*Gamma*sqrt(phot));
        U_disfb.set(bin[1](1),prime(bin[1](2)),bin[Nfb+1](phot+1),prime(bin[Nfb+1](phot  )),(-1.)*sqrt(dt)*Gamma*sqrt(phot));        
        U_fb.set(bin[1](2),prime(bin[1](1)),bin[2](phot  ),prime(bin[2](phot+1)),(-1.0*fb_fak)*sqrt(dt)*Gamma*exp(-Cplx_i*phi)*sqrt(phot));
        U_fb.set(bin[1](1),prime(bin[1](2)),bin[2](phot+1),prime(bin[2](phot  )),( 1.0*fb_fak)*sqrt(dt)*Gamma*exp(Cplx_i*phi)*sqrt(phot));
//         U_fb.set(bin[1](2),prime(bin[1](1)),bin[2](phot  ),prime(bin[2](phot+1)),(-1.0*fb_fak)*sqrt(dt)*Gamma*sqrt(phot));
//         U_fb.set(bin[1](1),prime(bin[1](2)),bin[2](phot+1),prime(bin[2](phot  )),( 1.0*fb_fak)*sqrt(dt)*Gamma*sqrt(phot));
    }//phot

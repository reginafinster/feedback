...
namespace itensor{
    ...
    void Feedback::apply_Uevo_with_feedback(int timestep)
    {
		...
		*res = MPS(N);
        ...
        if(timestep % 1 == 0) //ACHTUNG! Weiter unten wird noch entanglement ausgegeben!!
        {
        ...
        // calculate von Neumann entanglement in chain
        double total_entropy = 0.0;
        std::vector<double> entropy = std::vector<double>(N);
        for(int i=1;i<N;i++)
        {
            ITensor temp = MPS_chain.A(i)*MPS_chain.A(i+1);
            Index left_link = commonIndex(MPS_chain.A(i-1),MPS_chain.A(i));
            Index right_link = commonIndex(MPS_chain.A(i+1),MPS_chain.A(i+2));
            entropy[i-1] = 0.0;
            entropy[i-1] += Feedback::entanglement_entropy(temp,s[i],s[i+1],left_link,right_link);
            total_entropy += entropy[i-1];
        }
        
        fprintf(datastream_entanglement, "%.3f ", (timestep-Nfb+1)*dt);
        for (int i=(N-2);i>=0;i--) fprintf(datastream_entanglement, "\t %.5e ", entropy[i]);
        fprintf(datastream_entanglement, "\t %.5e \n", (total_entropy/double(N)) );
    }
    ...
    {	
		...
		*res = MPS(N);
        ...
        for(int i=1;i<N;i++)
        {
            temp=MPS_chain.A(i)*MPS_chain.A(i+1);
            Index left_link = commonIndex(MPS_chain.A(i-1),MPS_chain.A(i));
            Index right_link = commonIndex(MPS_chain.A(i+1),MPS_chain.A(i+2));
            entropy[i-1] = 0.0;
            entropy[i-1] += Feedback::entanglement_entropy(temp,s[i],s[i+1],left_link,right_link);
        ...
        fprintf(datastream_entanglement, "%.3f ", (timestep)*dt);
        for (int i=(N-2);i>=0;i--) fprintf(datastream_entanglement, "\t %.5e ", entropy[i]);
        fprintf(datastream_entanglement, "\t %.5e \n", (total_entropy/double(N)) );
    }

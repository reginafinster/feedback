...
namespace itensor{
    ...
    void Feedback::set_up_MPO()
    {
        ...
        // for identity in Taylor expansion
        ITensor identity_taylor = identity_sys_last*identity_bin*identity_Nfb*identity_sys_before_last;
		...
		for(int i=1;i<N;i++)
		{
			...
			{	
				...
				H_temp += Uevo;
                // Taylor expansion:
                // first order in system, second in interaction:
                H_temp = identity_taylor + H_temp + 0.5*mapprime(prime(Uevo)*Uevo,2,1);
//                 ITensor U_fb_1  = H_temp; 
...
//             
//                 H_temp = identity_taylor + U_fb_1 + U_fb_2 + U_fb_3 + U_fb_4 + U_fb_5 + U_fb_6 + U_fb_7 + U_fb_8 + U_fb_9 + U_fb_10 ;
...
//                 H_temp = identity_taylor + U_fb_1 + U_fb_2 + U_fb_3 + U_fb_4 + U_fb_5 + U_fb_6 + U_fb_7 + U_fb_8 + U_fb_9 + U_fb_10 ;
    ...
    void Feedback::apply_Uevo_with_feedback(int timestep)
    {
		...
		*res = MPS(N);
        ...
        delete res;
        if(timestep % 10 == 0) printf("time %.3f of %.3f -- norm_first_site=%.10f -- stagmag=%.10f -- occ_last_site=%.10f -- detector=%.10f\n",

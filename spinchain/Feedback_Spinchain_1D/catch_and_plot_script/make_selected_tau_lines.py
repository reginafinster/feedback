...
phi_selection_list = [0.0,0.5,1.0,1.5,2.0]
...
for m in range(len(sorted_valuelist)):
    ...
    if m != (len(sorted_valuelist)-1):
        if sorted_valuelist[m][3] != sorted_valuelist[m+1][3]:
            if sorted_valuelist[m][3] in phi_selection_list:
                phi_values.append(sorted_valuelist[m][3])
...
path_resultsfile = os.path.join(resultsdir,("selected_tau_lines.dat"))

...
path_resultsfile = os.path.join(resultsdir,("tau_line.dat"))
with open(path_resultsfile, 'w+') as f:
    f.write('#tau\t\t signal\n')
    for m in range(len(sorted_valuelist)):
        f.write("%f\t\t" % sorted_valuelist[m][2])
        f.write("%f\n" % sorted_valuelist[m][-5])
f.close()
...
#plt.imshow(normalized_detector_array, interpolation='none', aspect='auto', extent=[min_tau,max_tau,min_phi,max_phi], vmin=0, vmax=1)
#plt.legend()
#plt.xlabel('delay time tau')
#plt.ylabel('feedback phase $\phi$')
#plt.title('integrated detector signal')
